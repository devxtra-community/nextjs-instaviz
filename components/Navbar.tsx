    ...
    <nav className="fixed top-0 left-0 z-50 w-full bg-white/80 backdrop-blur-md border-b border-black/10">
      ...
      <div className="flex items-center space-x-6">
        <Link 
          ...
          <Link
            href="/signup"
            className="text-base font-medium text-white bg-violet-700 hover:bg-violet-800 rounded px-3 py-1.5 transition duration-200"
          >
            Signup
          </Link>
        </div>
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div
            ...
            className="md:hidden bg-white/95 backdrop-blur-sm border-t border-gray-100 shadow-sm overflow-hidden"
          >
            <div className="flex flex-col space-y-3 py-4 px-6 text-gray-700">
              <Link
                href="/"
                onClick={() => setMenuOpen(false)}
                className="text-base font-medium hover:text-violet-700 transition"
              >
                Dashboard
              </Link>
              <Link
